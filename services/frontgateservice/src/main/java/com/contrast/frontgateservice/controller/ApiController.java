...
public class ApiController {
    ...
    private ObjectInputFilter createAddressImportFilter() {
        return filterInfo -> {
            ...
            // Allow array types for safe classes
            if (clazz.isArray()) {
                Class<?> componentType = clazz.getComponentType();
                // Allow arrays of Object (used internally by collections) and primitive arrays
                ...
                // Allow arrays of java.util.* classes (e.g., HashMap$Node[])
                if (componentType.getName().startsWith("java.util.")) {
                    return ObjectInputFilter.Status.ALLOWED;
                }
                return ObjectInputFilter.Status.REJECTED;
            }
            // Allow standard Java utility classes (collections, etc.) - these are safe
